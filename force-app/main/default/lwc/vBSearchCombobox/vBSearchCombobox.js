...
export default class ReusableCustomDropdownWithSearchLwc extends LightningElement {
    ...
    set value(value) {
        console.log('Rendering new value: ', value);
        this._value = value;
        this.label = value;
    }
    ...
    @api setOptionsAndValues() {
        ...
        // console.log('Received options: ', this.optionsToDisplay);
        this.options.forEach(element => {
            if(typeof(element) == 'object'){
                this.optionsToDisplay.push({
                    label: element.label,
                    value: element.value
                });
            }
        ...
        if (this._value && this._value != "") {
            let label = this.getLabel(this._value);
            if (label && label != "") {
                this.label = label;
            }
        }
    ...
    get emptyOptions() {
        return (this.options && this.options.length > 0 ? false : true);
    }
