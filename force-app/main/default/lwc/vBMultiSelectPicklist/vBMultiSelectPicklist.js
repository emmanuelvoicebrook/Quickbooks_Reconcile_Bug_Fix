...
export default class MultiSelect extends LightningElement {
  ...
  @track options_ = [];
  // @track options_ = [{label:'Docksta table',value:'Docksta table',selected:false},
  ...
  set value(value){
    console.log('Setting value to: ', value + ' : ' +  typeof(value));
    this.value_ = value;
  ...
  renderedCallback(){
    var mainDiv = this.template.querySelector('.slds-combobox');
    //disable maindiv
    console.log('Disabling Main Div')
  ...
  */
  ...
  @api handleCloseFromParent(){
    console.log('Parent requesting close');
    if(this.isOpen){
      this.closeDropdown();
  ...
  handleSelectedClick(event){
    ...
    console.log('Check Shift', JSON.stringify(listData));
    console.log('Options before update: ', JSON.stringify(this.options_));
    // if (listData.shift) {
    //   this.options_.forEach(function(option) {
    ...
    console.log('Options after update: ', JSON.stringify(this.options_));
    this.selectedPills = this.getPillArray();
    console.log('Updated List', this.selectedPills);
    this.despatchChangeEvent();
